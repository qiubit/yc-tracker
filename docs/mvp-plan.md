...
- [x] Agent 03: Implement local search, FTS index, and optional semantic-search extension points.
- [x] Agent 04: Add analytics aggregation for batch, industry, tags, status, team size, hiring, and geography.
- [x] Agent 05: Build the dense explorer UI with filters, charts, results list/table, and company detail drawer.
- [x] Agent 06: Implement backend API routes and database access helpers using the shared schema.
- [x] Agent 07: Verify import, API, UI, search, analytics, and documentation end to end.
...
- [x] UI first screen is the explorer, not a landing page.
...
- [x] Detail view shows company source fields and optional enrichment when available.
- [x] Verification docs list all commands needed to run from a clean checkout and UI session.
...
- Local UI entry point: `scripts/ui-server.mjs` on `127.0.0.1:3000` by default, proxying `/api/*` to the API server.
